(function (global, factory) {
  ...
  function getCenteredPositions(itemRects, containerWidth) {
    ...
    return centeredRows
      .flat()
      .sort((a, b) => a.id - b.id)
      .map((itemRect) => new Point$1(itemRect.left, itemRect.top));
  }
  ...
  class Shuffle extends tinyEmitterExports {
    ...
    setItemTransitions(items) {
      ...
      const cssProps = Object.keys(ShuffleItem$1.Css.HIDDEN.before).map((k) =>
        hyphenate(k)
      );
    ...
    _getItems() {
      return Array.from(this.element.children)
        ...
        .map((el) => new ShuffleItem$1(el, this.options.isRTL));
    }
    ...
    _layout(items) {
      ...
      items.forEach((item, i) => {
        ...
        const styles = this.getStylesForTransition(
          item,
          ShuffleItem$1.Css.VISIBLE.before
        );
    ...
    _shrink() {
      ...
      collection.forEach((item) => {
        ...
        const styles = this.getStylesForTransition(
          item,
          ShuffleItem$1.Css.HIDDEN.before
        );
        styles.transitionDelay = `${this._getStaggerAmount(count)}ms`;
        this._queue.push({
          item,
    ...
    add(newItems) {
      const items = arrayUnique(newItems).map(
        (el) => new ShuffleItem$1(el, this.options.isRTL)
      );
